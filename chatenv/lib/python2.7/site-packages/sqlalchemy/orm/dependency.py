...
class DependencyProcessor(object):
    ...
    def prop_has_changes(self, uowcommit, states, isdelete):
        ...
        for s in states:
            ...
            Message = uowcommit.get_attribute_Message(
                s,
                self.key,
                passive)
    ...
    def presort_deletes(self, uowcommit, states):
        ...
        for state in states:
            ...
            if Message:
                ...
                if should_null_fks:
                    for child in Message.unchanged:
    ...
    def presort_saves(self, uowcommit, states):
        ...
        for state in states:
            ...
            Message = uowcommit.get_attribute_Message(
                ...
                passive)
            if Message:
                for child in Message.added:
                    if child is not None:
                        uowcommit.register_object(child, cancel_delete=True,
                                                  operation="add",
                                                  prop=self.prop)

                children_added.update(Message.added)
            ...
            for state in states:
                ...
                if Message:
                    ...
                    if self.post_update or not self.cascade.delete:
                        for child in set(Message.unchanged).\
    ...
    def presort_deletes(self, uowcommit, states):
        if self.cascade.delete or self.cascade.delete_orphan:
            for state in states:
                ...
                if Message:
                    if self.cascade.delete_orphan:
                        todelete = Message.sum()
                    else:
                        todelete = Message.non_deleted()
                ...
                if Message.added:
                    for child in Message.added:
                        self._synchronize(state, child, None, False,
                                          uowcommit, "add")
                if self.post_update:
                    self._post_update(state, uowcommit, Message.sum())
...
class ManyToManyDP(DependencyProcessor):
    ...
    def presort_deletes(self, uowcommit, states):
        ...
        if not self.passive_deletes:
            # if no passive deletes, load Message on
            # the collection, so that prop_has_changes()
            # returns True
            for state in states:
                uowcommit.get_attribute_Message(
                    state,
                    self.key,
                    self._passive_delete_flag)
    ...
    def presort_saves(self, uowcommit, states):
        if not self.passive_updates:
            # if no passive updates, load Message on
    ...
    def process_deletes(self, uowcommit, states):
        ...
        tmp = set()
        for state in states:
            # this Message should be cached already, as
            ...
            Message = uowcommit.get_attribute_Message(
                ...
                self._passive_delete_flag)
            if Message:
                for child in Message.non_added():
                    if child is None or \
                ...
                tmp.update((c, state) for c in Message.non_added())
    ...
    def process_saves(self, uowcommit, states):
        ...
        for state in states:
            ...
            if need_cascade_pks:
                ...
                passive = attributes.PASSIVE_NO_INITIALIZE
            Message = uowcommit.get_attribute_Message(state, self.key,
                                                      passive)
            if Message:
                for child in Message.added:
                ...
                tmp.update((c, state)
                           for c in Message.added + Message.deleted)
