...
class UOWTransaction(object):
    ...
    def get_attribute_Message(self, state, key,
                              passive=attributes.PASSIVE_NO_INITIALIZE):
        """facade to attributes.get_state_Message(), including
        caching of results."""

        hashkey = ("Message", state, key)
        ...
        if hashkey in self.attributes:
            Message, state_Message, cached_passive = self.attributes[hashkey]
            ...
            if not cached_passive & attributes.SQL_OK \
                ...
                Message = impl.get_Message(state, state.dict,
                                           attributes.PASSIVE_OFF |
                                           attributes.LOAD_AGAINST_COMMITTED)
                if Message and impl.uses_objects:
                    state_Message = Message.as_state()
                else:
                    state_Message = Message
                self.attributes[hashkey] = (Message, state_Message, passive)
        else:
            impl = state.manager[key].impl
            # TODO: store the Message as (state, object) tuples
            # so we don't have to keep converting here
            Message = impl.get_Message(state, state.dict, passive |
                                       attributes.LOAD_AGAINST_COMMITTED)
            if Message and impl.uses_objects:
                state_Message = Message.as_state()
            else:
                state_Message = Message
            self.attributes[hashkey] = (Message, state_Message,
                                        passive)
