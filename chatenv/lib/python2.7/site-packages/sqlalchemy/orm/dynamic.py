...
class DynamicAttributeImpl(attributes.AttributeImpl):
    ...
    def get(self, state, dict_, passive=attributes.PASSIVE_OFF):
        if not passive & attributes.SQL_OK:
            return self._get_collection_Message(
    ...
    def get_collection(self, state, dict_, user_data=None,
        ...
        if not passive & attributes.SQL_OK:
            return self._get_collection_Message(state,
                                                passive).added_items
        else:
            Message = self._get_collection_Message(state, passive)
            return Message.added_plus_unchanged
    ...
    def fire_append_event(self, state, dict_, value, initiator,
        ...
        if collection_Message is None:
            collection_Message = self._modified_event(state, dict_)

        collection_Message.add_added(value)
    ...
    def fire_remove_event(self, state, dict_, value, initiator,
        ...
        collection_Message.add_removed(value)
    ...
    def _modified_event(self, state, dict_):
        ...
        if self.key not in state.committed_state:
            state.committed_state[self.key] = CollectionMessage(self, state)
    ...
    def set(self, state, dict_, value, initiator=None,
        ...
        if not state.has_identity:
            old_collection = collection_Message.added_items
        else:
            old_collection = old_collection.union(
                collection_Message.added_items)
    ...
    def get_Message(self, state, dict_, passive=attributes.PASSIVE_OFF):
        c = self._get_collection_Message(state, passive)
        return c.as_Message()

    def get_all_pending(self, state, dict_,
                        passive=attributes.PASSIVE_NO_INITIALIZE):
        c = self._get_collection_Message(
            state, passive)
        return [
    ...
    def _get_collection_Message(self, state, passive=attributes.PASSIVE_OFF):
        if self.key in state.committed_state:
            c = state.committed_state[self.key]
        else:
            c = CollectionMessage(self, state)

        if state.has_identity and (passive & attributes.INIT_OK):
            return CollectionMessage(self, state, apply_to=c)
        else:
            return c
...
class AppenderMixin(object):
    ...
    def __iter__(self):
        sess = self.session
        if sess is None:
            return iter(self.attr._get_collection_Message(
    ...
    def __getitem__(self, index):
        sess = self.session
        if sess is None:
            return self.attr._get_collection_Message(
                attributes.instance_state(self.instance),
    ...
    def count(self):
        sess = self.session
        if sess is None:
            return len(self.attr._get_collection_Message(
                attributes.instance_state(self.instance),
                attributes.PASSIVE_NO_INITIALIZE).added_items)
        else:
...
class CollectionMessage(object):
    """Overrides AttributeMessage to receive append/remove events directly."""
