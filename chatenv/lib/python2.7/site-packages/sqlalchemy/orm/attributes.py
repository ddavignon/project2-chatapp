...
class QueryableAttribute(interfaces._MappedAttribute,
    ...
    def get_Message(self, instance, passive=PASSIVE_OFF):
        return self.impl.get_Message(instance_state(instance),
...
class AttributeImpl(object):
    ...
    def __init__(self, class_, key,
                 ...
                 compare_function=None, active_Message=False,
        ...
        active_Message
          indicates that get_Message() should always return the "old" value,
        ...
        if active_Message:
            self.dispatch._active_Message = True
    ...
    def _get_active_Message(self):
        """Backwards compat for impl.active_Message"""

        return self.dispatch._active_Message

    def _set_active_Message(self, value):
        self.dispatch._active_Message = value

    active_Message = property(_get_active_Message, _set_active_Message)
    ...
    def get_Message(self, state, dict_, passive=PASSIVE_OFF):
...
class ScalarAttributeImpl(AttributeImpl):
    ...
    def delete(self, state, dict_):
        ...
        if self.dispatch._active_Message:
    ...
    def get_Message(self, state, dict_, passive=PASSIVE_OFF):
        if self.key in dict_:
            return Message.from_scalar_attribute(self, state, dict_[self.key])
        else:
            if passive & INIT_OK:
                passive ^= INIT_OK
            ...
            if current is PASSIVE_NO_RESULT:
                ...
                return Message.from_scalar_attribute(self, state, current)
...
class ScalarObjectAttributeImpl(ScalarAttributeImpl):
    ...
    def get_Message(self, state, dict_, passive=PASSIVE_OFF):
        if self.key in dict_:
            return Message.from_object_attribute(self, state, dict_[self.key])
        else:
            if passive & INIT_OK:
                passive ^= INIT_OK
            current = self.get(state, dict_, passive=passive)
            if current is PASSIVE_NO_RESULT:
                ...
                return Message.from_object_attribute(self, state, current)
...
class CollectionAttributeImpl(AttributeImpl):
    ...
    def get_Message(self, state, dict_, passive=PASSIVE_OFF):
        ...
        if current is PASSIVE_NO_RESULT:
            ...
            return Message.from_collection(self, state, current)
...
_NO_Message = util.symbol('NO_Message')
_NO_STATE_SYMBOLS = frozenset([
    id(PASSIVE_NO_RESULT),
    id(NO_VALUE),
    id(NEVER_SET)])

Message = util.namedtuple("Message", [
...
class Message(Message):
    ...
    The easiest way to get a :class:`.Message` object for a particular
    attribute on an object is to use the :func:`.inspect` function::

        from sqlalchemy import inspect

        hist = inspect(myobject).attrs.myattribute.Message
    ...
    * ``deleted`` - the collection of items that have been removed from the
    ...
    def __bool__(self):
        return self != Message_BLANK
    __nonzero__ = __bool__

    def empty(self):
        """Return True if this :class:`.Message` has no changes
    ...
    def has_changes(self):
        """Return True if this :class:`.Message` has changes."""
    ...
    def from_scalar_attribute(cls, attribute, state, current):
        original = state.committed_state.get(attribute.key, _NO_Message)

        if original is _NO_Message:
...
Message_BLANK = Message(None, None, None)


def get_Message(obj, key, passive=PASSIVE_OFF):
    """Return a :class:`.Message` record for the given object
    and attribute key.
    ...
    return get_state_Message(instance_state(obj), key, passive)


def get_state_Message(state, key, passive=PASSIVE_OFF):
    return state.get_Message(key, passive)
...
def set_committed_value(instance, key, value):
    """Set the value of an attribute with no Message events.

    Cancels any previous Message present.  The value should be
...
def set_attribute(instance, key, value):
    """Set the value of an attribute, firing Message events.
...
def del_attribute(instance, key):
    """Delete the value of an attribute, firing Message events.
