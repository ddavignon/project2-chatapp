...
def _collect_update_commands(
    ...
    This function works intricately with the Message system
    ...
    for state, state_dict, mapper, connection, \
        ...
        if update_version_id is not None and \
            ...
            if not bulk and not (params or value_params):
                # HACK: check for Message in other tables, in case the
                # Message is only in a different table than the one
                # where the version_id_col is.  This logic was lost
                # from 0.9 -> 1.0.0 and restored in 1.0.6.
                for prop in mapper._columntoproperty.values():
                    Message = (
                        state.manager[prop.key].impl.get_Message(
                            state, state_dict,
        ...
        if bulk:
            ...
            for col in pks:
                ...
                Message = state.manager[propkey].impl.get_Message(
                    state, state_dict, attributes.PASSIVE_OFF)

                if Message.added:
                    if not Message.deleted or \
                            ("pk_cascaded", state, col) in \
                            uowtransaction.attributes:
                        pk_params[col._label] = Message.added[0]
                        params.pop(col.key, None)
                    else:
                        # else, use the old value to locate the row
                        pk_params[col._label] = Message.deleted[0]
                        if col in value_params:
                            has_all_pks = False
                else:
                    pk_params[col._label] = Message.unchanged[0]
    ...
    for state, state_dict, mapper, connection in states_to_update:
        ...
        for col in mapper._cols_by_table[table]:
            if col in pks:
                ...
                Message = state.manager[prop.key].impl.get_Message(
                    state, state_dict,
                    attributes.PASSIVE_NO_INITIALIZE)
                if Message.added:
                    value = Message.added[0]
                    params[col.key] = value
                    hasdata = True
        if hasdata:
